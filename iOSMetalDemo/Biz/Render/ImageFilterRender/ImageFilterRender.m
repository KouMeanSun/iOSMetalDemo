...
@implementation ImageFilterRender
{
    // 我们用来渲染的设备(又名GPU)
...
- (nonnull instancetype)initWithMetalKitView:(nonnull MTKView *)mtkView
{
    ...
    if(self)
    {
        ...
        // 从库中加载顶点函数
        id<MTLFunction> vertexFunction = [defaultLibrary newFunctionWithName:@"ImageFilterVertexShader"];
        
        // 从库中加载片段函数
        id<MTLFunction> fragmentFunction = [defaultLibrary newFunctionWithName:@"ImageFilterSamplingShader"];
...
{
    static const ImageFilterVertex quadVertices[] =
    {
        //像素坐标,纹理坐标
        { {  250,  -250 }, { 1.f, 0.f } },
        { { -250,  -250 }, { 0.f, 0.f } },
   ...
   // [renderEncoder setFragmentTexture:_texture atIndex:CCTextureIndexBaseColor];
    ...
    [computeEncoder setTexture:_inputTexture
                       atIndex:ImageFilterTextureIndexInput];
    //设置输出纹理
    [computeEncoder setTexture:_outputTexture
                       atIndex:ImageFilterTextureIndexOutput];
    ...
    if(renderPassDescriptor != nil)
    {
        ...
        [renderEncoder setVertexBytes:quadVertices
                              ...
                              atIndex:ImageFilterVertexInputIndexVertices];
        
        //将视图size-->顶点函数
        [renderEncoder setVertexBytes:&_viewportSize
                               length:sizeof(_viewportSize)
                              atIndex:ImageFilterVertexInputIndexViewportSize];
        //将输出纹理-->片段函数
