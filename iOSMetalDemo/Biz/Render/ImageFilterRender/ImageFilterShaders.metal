...
#include <simd/simd.h>
#import "ImageFilterShaderTypes.h"
...
ImageFilterVertexShader(uint vertexID [[ vertex_id ]],
             constant ImageFilterVertex *vertexArray [[ buffer(ImageFilterVertexInputIndexVertices) ]],
             constant vector_uint2 *viewportSizePointer  [[ buffer(ImageFilterVertexInputIndexViewportSize) ]])
...
fragment float4 ImageFilterSamplingShader(RasterizerData in [[stage_in]],
                               texture2d<half> colorTexture [[ texture(ImageFilterTextureIndexOutput) ]])
{
...
//灰度并行计算函数
kernel void
grayscaleKernel(texture2d<half, access::read>  inTexture  [[texture(ImageFilterTextureIndexInput)]],
                texture2d<half, access::write> outTexture [[texture(ImageFilterTextureIndexOutput)]],
