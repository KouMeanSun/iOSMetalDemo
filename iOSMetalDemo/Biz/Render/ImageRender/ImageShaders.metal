...
ImageVertexShader(uint vertexID [[vertex_id]],
             constant ImageVertex *vertexArray [[buffer(ImageVertexInputIndexVertices)]],
             constant vector_uint2 *viewportSizePointer [[buffer(ImageVertexInputIndexViewportSize)]])
{
    /*
     处理顶点数据:
...
fragment float4 ImageFragmentShader(RasterizerData in [[stage_in]],
                               texture2d<half> colorTexture [[texture(ImageTextureIndexBaseColor)]])
{
